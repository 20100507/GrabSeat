...
//		getSeat("http://172.16.47.84/Skip.aspx", "http://172.16.47.84/", "2014023097", "692138", "3207176");
//	}

	public static String getSeat(String appUrl, String cookieUrl, String username, String password, String seatId)
			throws Exception {
		...
		if(response.body().contains("明日预约成功")){
			Document parse = response.parse();
			Node node = parse.childNodes().get(1);
			Node node2 = node.childNodes().get(0);
			Node node3 = node2.childNodes().get(0);
			Node node4 = node3.childNodes().get(0);
			return node4.toString().trim();
		}
		return "座位已经被人抢了";
